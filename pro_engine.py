#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""PRO engine pre UnexplainedDaily reels (eerie dizajn: magenta akcent, tmavy grade, ARCHIVE sceny s realnymi fotkami artefaktov/miest z Wikimedia) (overeny na Plitvice demu, schvaleny 2026-07-04).
...
UA = {"User-Agent": "UnexplainedDaily/1.0 (educational mysteries channel)"}
REQUIRE_PLACE_MATCH = False   # mystery b-roll je atmosfericky; miesto drzi mapa+archiv
...
def render_overlay_seq(sc, idx, work):
    ...
    if role == "hook":
        ...
        chips[0] = {"img": chip_png("@UnexplainedDaily", 56, "accent"), "at": 0.35}
        chips[1] = {"img": chip_png("FOLLOW FOR MORE MYSTERIES", 42, "glass", FONT_POP), "at": 0.6}
    for fi in range(n):
        t = fi / FPS
...
# ---------------------------------------------------------------- render scen
GRADE = "eq=contrast=1.22:brightness=-0.08:saturation=0.68:gamma=0.88,vignette=angle=PI/3.8,noise=alls=6:allf=t"
